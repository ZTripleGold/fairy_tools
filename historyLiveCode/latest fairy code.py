...
# 模型选择（2026年5月）
# deepseek-v4-pro:  旗舰模型，当前2.5折活动（截止2026/05/31），适合高质量角色扮演
# deepseek-v4-flash: 极速便宜，适合高频弹幕快速回复
DEEPSEEK_MODEL_ID = "deepseek-v4-flash"

# 代理配置（DeepSeek 国内通常直连，无需代理）
PROXY_SETTINGS = {
    "http://": None,
    "https://": None,
}
...
# DeepSeek 推荐参数
DEEPSEEK_CONFIG = {
    "max_tokens": 256,      # 直播弹幕 256 完全够用，省 token 也省延迟
    "temperature": 1.0,     # DeepSeek 官方推荐默认值
    "top_p": 1.0,           # 官方推荐默认值
    "timeout": 60.0,
}
...
#9. 情景化：借景说理，用邦布、电费、主机散热、伊埃斯等录像店元素包装回答，必须原创，严禁直接模仿示例原句。
# ================================== 保留风格示例的 Fairy 人设（约 1200 tokens） ==================================
...
# ================================== 保留风格示例的 Youkai 人设（约 800 tokens） ==================================
...
tts_active = False
scan_paused = False      # 空格控制：扫码语录暂停
tts_paused = False       # Z键控制：TTS暂停
last_chat_reset_time = time.time()  # 定时对话计时器
...
async def scan_audio_loop():
    ...
    while True:
        ...
        if accumulated >= interval_ms:
            if tts_active:
                accumulated = interval_ms  # TTS占用中，保持满格等待
            else:
    ...
    while True:
        try:
            ...
            try:
                ...
                # 播放循环：检测 channel 是否还在播放
                while channel.get_busy():
                    if tts_paused:
                        # 用户按了 Z，stop() 已经被 keyboard_listener 调用
                        # 这里只需要等待恢复
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif event.unicode and event.unicode.lower() == 'z':    # 同时支持 Z 和 z
                    tts_paused = not tts_paused
                    if tts_paused:
                        pygame.mixer.stop()  # ← 立即停止所有 Sound
                        print("⏸️ TTS已手动暂停")
                    else:
                        print("▶️ TTS已继续")
    ...
    while True:
        try:
            ...
            if play_queue.qsize() > 0:
                ...
                last_chat_reset_time = time.time()  # 跳过也重置，避免死循环
                continue
